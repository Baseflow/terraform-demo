terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "demo/mla/production/kubernetes.tfstate"
  }
}
...
module "network" {
  ...
  vnet_address_space      = "10.222.0.0/15" # ranges 	10.222.0.1 - 10.223.255.254
  vnet_name               = "aks_vnet"
  subnets = [{
    name           = "aks-subnet"
    address_prefix = "10.222.0.0/16" # ranges 	10.222.0.1 - 10.222.255.254
    },
    {
      name           = "ingress-appgateway-subnet"
      address_prefix = "10.223.0.0/16" # ranges 	10.223.0.1 - 10.223.255.254
  }]
}
